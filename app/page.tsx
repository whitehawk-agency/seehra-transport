    ...
    <>
      ...
      <section className="relative min-h-[88vh] flex items-center justify-center overflow-hidden text-center">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 py-24">
          <h1 className="text-5xl lg:text-[82px] font-extrabold text-white leading-[1] mb-6 tracking-tight">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            <Link href="/contact" className="bg-[#f7680b] hover:bg-[#e55a00] text-white px-10 py-4 rounded-xl font-bold text-base transition-all hover:-translate-y-0.5 hover:shadow-lg hover:shadow-orange-500/30">Get a Quote →</Link>
            <Link href="/services" className="border-2 border-white/50 hover:border-white text-white px-10 py-4 rounded-xl font-bold text-base transition-all hover:bg-white/10">Our Services</Link>
          </div>
        </div>
      </section>
      ...
      <div className="bg-[#0a0a0a] border-t border-white/5 py-4 overflow-hidden">
        <div className="flex animate-marquee whitespace-nowrap">
          {Array(4).fill(["Multi-Drop Delivery","Last-Mile Logistics","Same-Day Courier","Next-Day Express","Flexible Solutions","98% On-Time","GPS Tracked","Fully Insured","Nationwide UK","West Midlands Based","From £4.99","On Time. Every Time."]).flat().map((item, i) => (
            <span key={i} className={`mx-8 text-xs font-bold tracking-widest uppercase ${i % 2 === 0 ? "text-[#f7680b]" : "text-white/20"}`}>{item}</span>
          ))}
        </div>
      </div>

      {/* ── OUR STORY — one quote button only, quote widget below ── */}
      <section className="py-24 px-6 bg-white" id="about">
        <div className="max-w-4xl mx-auto">
          <p className="text-[#f7680b] text-xs font-bold tracking-widest uppercase mb-4 text-center">Who We Are</p>
          <h2 className="text-4xl lg:text-5xl font-extrabold leading-tight mb-6 text-center">
            Invisible every day,{" "}
          ...
          <p className="text-gray-600 leading-relaxed mb-5 text-base">
            Over a decade of experience has allowed us to build a reputation on three things: reliability, efficiency, and professionalism. Our team of drivers and logistics experts work tirelessly so your deliveries arrive on time — and your customers' experience reflects well on your brand.
          </p>
          ...
          {/* Single "Our Story" button — quote is in the widget below */}
          <div className="mb-12">
          ...
          {/* Quote widget — this is the one and only quote CTA on this section */}
          <QuoteWidget />
        </div>
      </section>

      {/* ── SERVICES — all equal size cards ── */}
      <section className="bg-gray-50 py-24 px-6" id="services">
        <div className="max-w-7xl mx-auto">
          ...
          <h2 className="text-4xl lg:text-5xl font-extrabold leading-tight text-center mb-3">All roads lead to your door</h2>
          <p className="text-gray-500 text-sm leading-relaxed text-center max-w-xl mx-auto mb-12">
          ...
          {/* All cards identical — 3 columns, no wide/spanning card */}
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {services.map((s) => (
              <Link key={s.title} href={s.href}
                className="group bg-white rounded-2xl p-7 border border-gray-100 hover:border-orange-200 hover:shadow-xl hover:shadow-orange-500/5 transition-all hover:-translate-y-1 flex flex-col">
                <div className="text-4xl mb-4">{s.icon}</div>
                <h3 className="text-xl font-bold mb-2 group-hover:text-[#f7680b] transition-colors">{s.title}</h3>
      ...
      <section className="py-24 px-6 bg-white">
        <div className="max-w-7xl mx-auto">
          ...
          <h2 className="text-4xl font-extrabold text-center mb-3">Simple, Fair Rates</h2>
          <p className="text-gray-500 text-center mb-12 max-w-lg mx-auto text-sm">No hidden fees. No surprises. Volume discounts available for businesses.</p>
          ...
          <p className="text-center text-gray-400 text-xs mt-6">All prices exclude VAT. <Link href="/contact" className="text-[#f7680b] underline">Contact us</Link> for volume discounts and custom quotes.</p>
        </div>
      </section>

      {/* ── RECRUITMENT TEASER ── */}
      <section className="py-20 px-6" style={{ background:"linear-gradient(135deg,#e62b1e 0%,#f7680b 100%)" }}>
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-8">
          <div>
            <h2 className="text-4xl font-extrabold text-white leading-tight mb-3">Drive with Seehra.</h2>
            <p className="text-white/80 text-lg max-w-xl">Hiring self-employed delivery drivers for East & West Midlands routes. Weekly pay, training provided, fuel paid on top of your day rate.</p>
          </div>
          <Link href="/recruitment" className="bg-white text-[#f7680b] px-8 py-4 rounded-xl font-bold text-base whitespace-nowrap hover:bg-orange-50 transition-colors shadow-xl">View Careers →</Link>
        </div>
      </section>
    </>
  );
